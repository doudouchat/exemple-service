...
public class AuthorizationLoginTest extends JerseySpringSupport {
    ...
    public void authorizedGetLoginWithSecond() throws Exception {
        ...
        Mockito.when(loginService.get(Mockito.eq("jack_doe"), Mockito.eq("test"), Mockito.eq("v1"))).thenReturn(JsonNodeUtils.create(model));
        ...
        Mockito.verify(loginService).get(Mockito.eq("jack_doe"), Mockito.eq("test"), Mockito.eq("v1"));
